...
var resizePizzas = function(size) { 
  ...
  /**
  * clean up a pizza element so we can resize appropriately
  * @elem {object} randomPizzaContainer element 
  **/
  function cleanClasses(elem) {
    elem.classList.remove('smallPizza', 'mediumPizza', 'largePizza')
    return elem;
  }

  /** 
  * Iterates through pizza elements on the page and changes their widths using CSS classes
  * @size {string} value passed in by the slider
  **/
  function changePizzaSizes(size) {
    var _pizzas = document.querySelectorAll(".randomPizzaContainer");
  
    for (var i = 0; i < _pizzas.length; i++) {
      var _elem = _pizzas[i];

      // clean up existing pizza css classes
      _elem = cleanClasses(_elem)

      switch(size) {
        case "1":
          _elem.classList.add("smallPizza");
          break;
        case "2":
          _elem.classList.add("mediumPizza");
          break;
        case "3":
          _elem.classList.add("largePizza");
          break;
        default:
          _elem.classList.add("mediumPizza");
          break;
...
// way to store the mover elements
var items = [];
/** 
* Moves the sliding background pizzas based on scroll position
* @timestamp {number} timing offset passed from rAF.js
**/
function updatePositions(timestamp) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    var phase = Math.sin((timestamp / 1250) + (i % 5));
...
var raf;
// using shim in rAF.js
function scrollAnimation() {
  // our callback is updatePositions()
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  var _totalMovers = (_height / 100) * cols;

  for (var i = 0; i < _totalMovers; i++) {
